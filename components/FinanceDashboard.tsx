  ...
  return (
    <div className="h-full overflow-y-auto p-4 md:p-8 space-y-8 relative">
      ...
      {isAddingTransaction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 rounded-2xl shadow-2xl relative">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6">Add Detailed Entry</h3>
              <form onSubmit={handleAddManualTransaction} className="space-y-4">
                 <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                        <button 
                           ...
                           className={`py-2 rounded-lg border text-sm font-medium transition-colors ${manualTxType === 'expense' ? 'bg-red-500/10 border-red-500 text-red-600 dark:text-red-200' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400'}`}
                        >
                            Expense
                        </button>
                         <button 
                           ...
                           className={`py-2 rounded-lg border text-sm font-medium transition-colors ${manualTxType === 'income' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-600 dark:text-emerald-200' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400'}`}
                        >
                            Income
                        </button>
                    </div>
                 ...
                 {/* Income Source Selector (Only for Income) */}
                 {manualTxType === 'income' && (
                   <div className="mb-4">
                      <label className="block text-sm font-medium text-emerald-600 dark:text-emerald-400 mb-1">Income Source</label>
                      <select
                        value={selectedIncomeSourceId}
                        onChange={(e) => handleIncomeSourceSelect(e.target.value)}
                        className="w-full bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                      >
                 ...
                 <div>
                    <label className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Merchant / Description</label>
                    <input 
                      type="text" 
                      required
                      value={manualTxMerchant}
                      onChange={(e) => setManualTxMerchant(e.target.value)}
                      className="w-full bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    />
                 </div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Recurrence</label>
                        <select 
                            value={manualTxRecurrence}
                            onChange={(e) => setManualTxRecurrence(e.target.value as any)}
                            className="w-full bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                        >
                 ...
                 <button type="submit" className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-semibold">
                    Add Transaction
                 </button>
              </form>
           </div>
      ...
      {isManagingSubscriptions && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-lg p-6 rounded-2xl shadow-2xl relative max-h-[85vh] flex flex-col">
              ...
              <div className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <CalendarClock className="w-6 h-6 text-indigo-500 dark:text-indigo-400" />
                    Subscriptions & Recurring
                </h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">Manage automatic payments for Netflix, Rent, Salaries, etc.</p>
              </div>

              {/* Add New Subscription Form */}
              <form onSubmit={handleAddSubscription} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl mb-6 border border-slate-200 dark:border-slate-700/50">
                 <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Add New</h4>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                    <input 
                        ...
                        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500"
                    />
                    <input 
                        type="number" 
                        required
                 ...
                 <div className="grid grid-cols-2 gap-3 mb-3">
                    <select 
                        ...
                        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none"
                    >
                        <option value="expense">Expense</option>
                        <option value="income">Income</option>
                    </select>
                 ...
                 <div className="grid grid-cols-2 gap-3 mb-3">
                    <select 
                        ...
                        onChange={(e) => setNewSubFrequency(e.target.value as any)}
                        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none"
                    >
                        <option value="weekly">Weekly</option>
                        <option value="monthly">Monthly</option>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                 {subscriptions.map(sub => (
                    <div key={sub.id} className={`p-3 rounded-xl border ${sub.status === 'active' ? 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700' : 'bg-slate-100 dark:bg-slate-800/50 border-slate-200 dark:border-slate-800 opacity-70'} flex items-center justify-between group`}>
                        <div className="flex items-center gap-3">
                             <div className={`w-10 h-10 rounded-full flex items-center justify-center ${sub.type === 'income' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-500' : 'bg-rose-500/10 text-rose-600 dark:text-rose-500'}`}>
                                <Repeat className="w-5 h-5" />
                             </div>
                             <div>
                                <h5 className="font-semibold text-slate-900 dark:text-white">{sub.name}</h5>
                        ...
                        <div className="flex items-center gap-4">
                            <span className={`font-bold ${sub.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-200'}`}>
                            ...
                            <div className="flex gap-1">
                                ...
                                >
                                    {sub.status === 'active' ? <PauseCircle className="w-5 h-5" /> : <PlayCircle className="w-5 h-5" />}
                                </button>
                                <button 
      ...
      {isManagingBudgets && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 rounded-2xl shadow-2xl relative">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Manage Monthly Budgets</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Set monthly spending limits for categories.</p>

                <form onSubmit={handleSetBudget} className="space-y-4 mb-6">
                    <div>
                        <label className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Category</label>
                        <select 
                ...
                <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                    {budgets.map(budget => (
                        <div key={budget.id} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg group">
                            <div>
                                <p className="text-slate-700 dark:text-slate-200 font-medium">{budget.category}</p>
                                <p className="text-xs text-slate-500">Limit: ₱{budget.amount.toLocaleString()}</p>
                            </div>
                            <button 
                                onClick={() => onRemoveBudget(budget.id)}
                                className="text-slate-400 hover:text-red-400 p-2 opacity-0 group-hover:opacity-100 transition-all"
                            >
                                <Trash2 className="w-4 h-4" />
                            </button>
                        </div>
                    ...
                    {budgets.length === 0 && (
                        <p className="text-center text-slate-500 text-sm py-4">No budgets set yet.</p>
                    )}
                </div>
            </div>
        </div>
      ...
      {isManagingCategories && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 rounded-2xl shadow-2xl relative max-h-[80vh] flex flex-col">
              ...
              <form onSubmit={handleAddCategory} className="flex gap-2 mb-6">
                <input 
                    type="text" 
                    placeholder="New category... (add emoji!)"
                    value={newCategoryName}
                    onChange={(e) => setNewCategoryName(e.target.value)}
                    className="flex-1 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                />
                <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 rounded-lg">
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {categories.map((cat) => (
                    <div key={cat} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg group">
      ...
      {/* Income Source Management Modal */}
      {isManagingIncomeSources && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 rounded-2xl shadow-2xl relative max-h-[80vh] flex flex-col">
              <button onClick={() => setIsManagingIncomeSources(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-900 dark:hover:text-white">
                 <X className="w-5 h-5" />
              </button>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Manage Income Sources</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Define where your money comes from.</p>
              
              <form onSubmit={handleAddIncomeSource} className="space-y-4 mb-6">
                 <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Source Name</label>
                    <input 
                        type="text" 
                        required
                        placeholder="e.g. Consulting Client A"
                 ...
                 <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Default Category</label>
                    <div className="flex gap-2">
                        {isCreatingCategory ? (
                          <div className="flex-1 flex gap-2 animate-in fade-in">
                            <input 
                                ...
                                className="flex-1 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm"
                            />
                            <button 
                                type="button" 
                                onClick={handleQuickAddCategory}
                                className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 rounded-lg"
                                title="Save Category"
                            >
                            ...
                            <button 
                                ...
                                className="bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-white px-3 rounded-lg"
                                title="Cancel"
                            >
                                <X className="w-4 h-4" />
                            </button>
                          ...
                          <>
                            <select 
                                value={newIncomeSourceCategory}
                                onChange={(e) => setNewIncomeSourceCategory(e.target.value)}
                                className="flex-1 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                            >
                                {categories.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                            <button 
                                ...
                                className="bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-white px-3 rounded-lg border border-slate-300 dark:border-slate-600"
                            >
                                <Plus className="w-4 h-4" />
                            </button>
                            <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 rounded-lg" title="Add Income Source">
                                <Plus className="w-5 h-5" />
                            </button>
                          </>
                        )}
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {incomeSources.map((source) => (
                    <div key={source.id} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg group border border-slate-200 dark:border-slate-800">
                        {editingSourceId === source.id ? (
                          <div className="flex-1 flex items-center gap-2 animate-in fade-in">
                            <span className="text-slate-700 dark:text-slate-200 text-sm font-medium w-1/3 truncate">{source.name}</span>
                            <select 
                              value={editSourceCategory}
                              onChange={(e) => setEditSourceCategory(e.target.value)}
                              className="flex-1 bg-slate-200 dark:bg-slate-700 border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white rounded px-2 py-1 text-sm focus:outline-none"
                            >
                                {categories.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                            <button onClick={() => saveEditedSource(source)} className="text-emerald-500 hover:text-emerald-400 p-1"><Check className="w-4 h-4" /></button>
                            <button onClick={() => setEditingSourceId(null)} className="text-slate-400 hover:text-slate-500 p-1"><X className="w-4 h-4" /></button>
                          </div>
                        ) : (
                          <>
                            <div>
                                <p className="text-slate-700 dark:text-slate-200 font-medium">{source.name}</p>
                                <p className="text-xs text-slate-500">{source.category}</p>
                            </div>
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                              <button 
                                  onClick={() => startEditingSource(source)}
                                  className="text-slate-500 hover:text-indigo-500 dark:hover:text-indigo-400"
                                  title="Edit category"
                              >
                ...
                {incomeSources.length === 0 && (
                    <p className="text-center text-slate-500 text-sm py-4">No sources defined.</p>
                )}
              </div>
           </div>
        </div>
      ...
      {isAddingGoal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 rounded-2xl shadow-2xl relative">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Set Micro-Goal</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Small steps lead to big savings!</p>
                
                <form onSubmit={handleCreateGoal} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Goal Name</label>
                        <input 
                            type="text" 
                            required
                            value={newGoalTitle}
                            onChange={(e) => setNewGoalTitle(e.target.value)}
                            placeholder="e.g. New Shoes 👟, Concert Tix 🎫"
                            className="w-full bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Target Amount (₱)</label>
                        <input 
                            type="number" 
                            required
                            min="1"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Target Date</label>
                        <input 
                            type="date" 
                            required
                            value={newGoalDate}
                    ...
                    <button 
                        ...
                        className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-semibold transition-all shadow-lg shadow-indigo-500/20 disabled:opacity-50 flex justify-center items-center gap-2"
                    >
                        {isGeneratingPlan ? (
                            <>
                            ...
                            <>
                                <Sparkles className="w-5 h-5" /> Start Saving
                            </>
                        )}
                    </button>
                </form>
